...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.textEdit.setFixedWidth(724)
        ...
        self.formatHeader1.setLineHeight(130, QTextBlockFormat.ProportionalHeight)

        self.formatParagraph = QTextBlockFormat()
        self.formatParagraph.setBottomMargin(18)
        self.formatParagraph.setLineHeight(130, QTextBlockFormat.ProportionalHeight)
    ...
    def markdownWithEmptyLines(self):
        ...
        text = re.sub(r'(?<!\n)\n(?!\n|\s*(\d+\.|-|\*)\s+)', ' ',self.textEdit.toMarkdown())
    ...
    def loadWithEmptyLines(self, file):
        ...
        for i, line in enumerate(lines):
            if re.match(r'^\s+$', line):  # If the line contains only spaces
                line = re.sub(r'\s{1,}', '<p> </p>', line)  # Replacing multiple spaces with a single space
                lines[i] = line
            elif line.strip() == "#":
                lines[i] = "<h1> </h1>"
    ...
    def loadWithEmptyLinesPDF(self, text):
        ...
        for i, line in enumerate(lines):
            if re.match(r'^\s+$', line):  # If the line contains only spaces
                line = re.sub(r'\s{1,}', '<p><br></p>', line)  # Replacing multiple spaces with a single space
                lines[i] = line
            elif line.strip() == "#":
                lines[i] = "<h1><br></h1>"
    ...
    def sizePic(self, markdownText):
        ...
        # Set the width of each image to 600
        for img in imgs:
    ...
    def insertPicture(self):
        ...
        if filePath:
            ...
            # Setting the maximum width of the label corresponding to the width of the QTextEdit
            ...
            if event.key() == Qt.Key_Delete or event.key() == Qt.Key_Backspace:
                ...
                if cursor.positionInBlock() == 0: #format.headingLevel() == 1
                    ...
                    if previusBlock.isValid():
                        cursor.beginEditBlock()
                        previousCharFormat = previusBlock.charFormat() #undo() protection
    ...
    def handleBlockCountChanged(self, newBlockCount):
        if newBlockCount > self.blockCount:
            # Finds a block that has been deleted
