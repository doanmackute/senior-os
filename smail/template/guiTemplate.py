    ...
    def createOPT1Buttons(self):
        ...
        for i in self.option:
            ...
            if i == 1:  # make first button exit button
                ...
                self.button_dict[i] = Button(self.optionsBar, image=self.person1Image, compound="c",
                                             command=execCommand)
            elif i ==4:
                self.button_dict[i] = Button(self.optionsBar, image=self.person2Image, compound="c",
                                             command=execCommand)
            else:
                self.button_dict[i] = Button(self.optionsBar, image=self.dummyPixel, compound="c",
                                             command=execCommand)
    ...
    def createOPT2Buttons(self):
        ...
        for i in self.option:
            def execCommand(x=i):  # this def stores current i of each button
                ...
                executeCommandFromOPTButton(x)
            if i ==1:
                self.button_dict[i] = Button(self.optionsBar, image=self.person3Image, compound="c", command=execCommand)
            elif i ==2:
                self.button_dict[i] = Button(self.optionsBar, image=self.person4Image, compound="c", command=execCommand)
            elif i ==3:
                self.button_dict[i] = Button(self.optionsBar, image=self.person5Image, compound="c", command=execCommand)
            elif i ==4:
                self.button_dict[i] = Button(self.optionsBar, text=self.text[f"smail_{self.language}_sendToButton"], image=self.dummyPixel, compound="c", command=execCommand)
            else:
                self.button_dict[i] = Button(self.optionsBar, text="default", image=self.dummyPixel, compound="c", command=execCommand)
